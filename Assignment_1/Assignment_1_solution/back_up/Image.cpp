...
vector<unsigned int> Image::operator[] (int p_index) {
	...
	/*if (p_index < 0) {
		cout << "pixel index out of range! You got the first pixel." << endl;
		p_index = 0;
	}
	else if (p_index >= this->image->msize) {
		cout << "pixel index out of range! You got the tail pixel." << endl;
		p_index = this->image->msize - 1;
	}*/
	vector<unsigned int> res(this->image->channel, 0);
	//unsigned int* res = new unsigned int[this->image->channel];
